...
export default function AdminPostings() {
    const { props } = usePage<{ posts: PaginatedPostings; allPostDates: string[] }>();
    const postsData = props.posts;
    const [posts, setPosts] = React.useState<CareerPost[]>(postsData?.data as CareerPost[] ?? []);
    const [isInitialLoading, setIsInitialLoading] = React.useState(true);
    const [views, setViews] = useState<number>(0);

    // Sync local state with props when page loads
    React.useEffect(() => {
        setPosts(postsData?.data as CareerPost[] ?? []);
    }, [postsData]);
    ...
    useEffect(() => {
        fetch('/views/track', {
            ...
            body: JSON.stringify({ page: 'admin-postings' }),
            credentials: 'same-origin',
        })
            .then(() => fetch('/views/count?page=admin-postings'))
            .then((r) => r.json())
    ...
    // Pagination: navigate pages like dashboard
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        console.log('Admin Submit function called');
        ...
        if (editing) {
            ...
            axios
                .post(`/admin/postings/${editing.id}`, fd, {
                    headers: {
            ...
            console.log('Sending POST request to /admin/postings');
    ...
    const confirmDelete = () => {
        if (postToDelete) {
            ...
            axios
                .delete(`/admin/postings/${postToDelete.id}`, {
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
                    },
    ...
    return (
        <>
            <AppLayout breadcrumbs={breadcrumbs} searchTerm={searchTerm} onSearchChange={setSearchTerm}>
                <Head title="Admin - Postings Management" />
                            ...
                            <div className="space-y-4 md:col-span-2">
                                ...
                                <Card>
                                    <CardContent className="p-4">
                                            ...
                                            {isLoading || isInitialLoading ? (
                                                ...
                                                (() => {
                                                    const displayed: Posting[] = isSearching
                                                        ...
                                                        : (filteredPosts as unknown as Posting[]);
                                                    if (displayed.length === 0 && posts.length > 0) {
                                                        return null;
                                                    }
                                                    return (
                                                        <PostingList
                                        ...
                                        {/* Pagination Controls (inside card like dashboard) */}
                                        <Pagination postsData={postsData as any} onPageChange={handlePageChange} isSearching={isSearching} />
                                    </CardContent>
                                </Card>
                ...
                <DialogContent className="h-[85vh] w-full max-w-[1000px] overflow-y-auto" style={{ maxWidth: '1000px' }}>
                    ...
                    <AdminPostingForm
                        ...
                        setData={(key: any, value: any) => {
                            console.log('Admin setData called:', key, value);
